class ProductsController < ApplicationController
  ...
  before_action :authorize_only_admins, except: [ :pastries_index, :delicacies_index, :show ]

  def pastries_index
    @categories = Category.joins(:products).where('products.supercategory' => 'Pastries').distinct
    @products = Product.where('supercategory' => 'Pastries').distinct
    if params[:filters]
      if filter_params[:range_min] && filter_params[:range_max]
        @products = @products.where('price >= ?', filter_params[:range_min]).where('price <= ?', filter_params[:range_max])
      end
  ...
  def delicacies_index
    @categories = Category.joins(:products).where('products.supercategory' => 'Delicacies').distinct
    @products = Product.where('supercategory' => 'Delicacies')
    if params[:filters]
      if filter_params[:range_min] && filter_params[:range_max]
        @products = @products.where('price >= ?', filter_params[:range_min]).where('price <= ?', filter_params[:range_max])
      end
  ...
  private
     def product_params
        params.require(:product).permit(:name, :price, :description, :supercategory, product_images_attributes: [ :image ], sizes: [], product_categories_attributes: [ :category_id ])
     end
